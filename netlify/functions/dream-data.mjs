...
export default async(req)=>{
  ...
  if(req.method==="GET"){
    let [data,visitors]=await Promise.all([store.get(DATA_KEY,{type:"json"}),store.get(VISITOR_KEY,{type:"json"})]);
    if(!data){data=structuredClone(INITIAL_DATA);await store.setJSON(DATA_KEY,data)}
  ...
  if(req.method==="POST"){
    ...
    if(body?.action==="contribution"){
      ...
      const goal=current.goals.find(g=>String(g.id)===safeText(body.goalId,80));
      ...
      current.transactions.unshift({name:safeText(body.name,80)||"Anoniem",amount,message:safeText(body.message,280),goal:safeText(goal.title,120),cash:Boolean(body.cash),photo});
