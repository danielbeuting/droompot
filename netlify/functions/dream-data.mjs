...
async function readState(supabase, slug) {
  const { data: pot, error: potError } = await supabase
    .from('dreampots')
    .select('id,child_name,birth_date,theme,photo_path,general_savings_description,is_public')
    .eq('slug', slug)
    .eq('is_public', true)
    .maybeSingle()
  ...
  const [{ data: goals, error: goalError }, { data: wishes, error: wishError }, { data: contributions, error: contributionError }] = await Promise.all([
    ...
    supabase.from('contributions').select('id,giver_name,amount,message,method,photo_path,goal_id,created_at').eq('dreampot_id', pot.id).eq('status', 'completed').order('created_at', { ascending: false }).limit(50),
  ])
  ...
  const mappedGoals = (goals || []).map(g => ({
    id: g.id,
    title: g.title,
    icon: g.icon || (g.is_general ? '💰' : '🎯'),
    type: g.is_general ? 'Spaarrekening' : 'Spaardoel',
    description: g.description || (g.is_general ? pot.general_savings_description || 'Vrij sparen voor later.' : ''),
    current: Number(g.current_amount || 0),
    goal: Number(g.target_amount || 1),
  }))
  ...
  return {
    ...
    wishes: (wishes || []).map(w => ({
      ...
      link: w.product_url || '',
      note: w.description || '',
      image: publicMedia(supabase, w.image_url),
      emoji: '🎁',
      claimed: Boolean(w.claimed),
    })),
    transactions: (contributions || []).map(c => ({
      id: c.id,
      name: c.giver_name || 'Anoniem',
      amount: Number(c.amount || 0),
      message: c.message || '',
      goal: goalById.get(c.goal_id) || 'Droompot',
      cash: c.method === 'cash',
      photo: publicMedia(supabase, c.photo_path),
    })),
  }
}
...
async function writeState(supabase, slug, state) {
  ...
  if (nonGeneralIds.length) {
    const { error } = await supabase.from('savings_goals').delete().in('id', nonGeneralIds)
    if (error) throw error
  }
  ...
  if (general && generalInput) {
    const { error } = await supabase.from('savings_goals').update({
      title: generalInput.title || 'Algemene spaarrekening',
      description: generalInput.description || 'Vrij sparen voor later.',
  ...
  const normalGoals = incomingGoals.filter(g => g !== generalInput).map((g, i) => ({
    dreampot_id: pot.id,
    title: String(g.title || 'Spaardoel'),
    description: g.description || null,
    icon: g.icon || '🎯',
    target_amount: Math.max(1, Number(g.goal || 1)),
    current_amount: Math.max(0, Number(g.current || 0)),
    is_general: false,
    sort_order: i,
  }))
  if (normalGoals.length) {
    const { error } = await supabase.from('savings_goals').insert(normalGoals)
    if (error) throw error
  }
  ...
  const wishes = (Array.isArray(state.wishes) ? state.wishes : []).map((w, i) => ({
    dreampot_id: pot.id,
    title: String(w.title || 'Cadeau'),
    description: w.note || null,
    price: Math.max(0, Number(w.price || 0)),
    product_url: w.link || null,
    image_url: w.image || null,
    claimed: Boolean(w.claimed),
    claimed_at: w.claimed ? new Date().toISOString() : null,
    sort_order: i,
  }))
  if (wishes.length) {
    const { error } = await supabase.from('wishlist_items').insert(wishes)
    if (error) throw error
  }
...
async function addContribution(supabase, slug, body) {
  ...
  const { error } = await supabase.rpc('add_public_contribution', {
    ...
    target_goal: goal.id,
    p_giver_name: String(body.name || 'Anoniem').slice(0, 40),
    p_amount: amount,
    p_message: String(body.message || '').slice(0, 120),
