...
const LOCAL_OPEN_COUNT_KEY="droompot-v17-open-count";
function registerLocalOpen(){
  const next=(Number(localStorage.getItem(LOCAL_OPEN_COUNT_KEY))||0)+1;
  localStorage.setItem(LOCAL_OPEN_COUNT_KEY,String(next));
  return next;
}
const LOCAL_OPEN_COUNT=registerLocalOpen();
const VISITOR_ID_KEY="droompot-v17-visitor-id";
...
async function registerUniqueVisitor(){
  try{
    const response=await fetch("/.netlify/functions/dream-data",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({action:"visit",visitorId:getVisitorId()})
    });
    if(!response.ok)return;
    const payload=await response.json();
    const el=document.getElementById("unique-visitor-count");
    if(el && Number.isFinite(Number(payload.uniqueVisitors)))el.textContent=String(payload.uniqueVisitors);
  }catch{}
}
...
async function loadCentralState(){
  try{
    ...
    if(visitorEl && Number.isFinite(Number(payload?.uniqueVisitors)))visitorEl.textContent=String(payload.uniqueVisitors);
...
function updateSyncStatus(){
  ...
  text.textContent=centralStorageAvailable
    ...
    :"Backend tijdelijk niet bereikbaar · lokale fallback actief";
}
...
function isDemoContribution(t){
  if(t?.demo===true)return true;
  return DEFAULT.transactions.some(d=>
    d.name===t?.name &&
    Number(d.amount)===Number(t?.amount) &&
    d.message===t?.message &&
    d.goal===t?.goal &&
    Boolean(d.cash)===Boolean(t?.cash)
  );
}
...
async function fileToCompressedDataURL(file,maxSize=320,quality=.78){
  // createImageBitmap with imageOrientation:"from-image" respects EXIF orientation
  // and avoids accidental mirroring/rotation on mobile camera photos.
  try{
    ...
    // Explicitly reset transforms so the image can never be mirrored.
...
function render(){
  setTimeout(syncDynamicChildNames,0);
  ...
  const localOpenCount=document.getElementById("local-open-count");
  if(localOpenCount)localOpenCount.textContent=String(LOCAL_OPEN_COUNT);
  ...
  state.goals.forEach((g,i)=>{
    ...
    if(g.id==="account"){
      card.innerHTML=`<div><div class="goal-icon">${g.icon}</div><div class="goal-type">${esc(g.type)}</div><h3>${esc(g.title)}</h3>
        ...
        <div class="general-badge">✨ Bijdragen aan ${state.childName}'s toekomst</div>`;
    }else{
...
function renderTransactions(){
  ...
  state.transactions.slice(0,8).forEach(t=>{
    ...
    x.innerHTML=`${avatar}<div><div class="transaction-name">${esc(t.name)}${isDemoContribution(t)?` <span class="demo-label">(demo)</span>`:""}${typeTag}</div><div class="transaction-message">${esc(t.message||"Heeft bijgedragen")}</div><div class="transaction-goal">${esc(t.goal||"Droompot")}</div></div><div class="transaction-amount">+ ${euro(t.amount)}</div>`;
    list.appendChild(x);
  })
}
function renderSettingsGoals(){
  const generalGoal=(settingsDraft.goals||[]).find(g=>g.id==="general");
  const generalDescription=document.getElementById("setting-general-description");
  if(generalDescription) generalDescription.value=generalGoal?.description||"Een potje voor later, zonder vast doel. Elke bijdrage groeit mee met de toekomst.";
...
async function complete(cash){
  ...
  try{
    ...
    if(!response.ok)throw new Error("Contribution save failed");
    const payload=await response.json();
    if(payload?.data)afterState={...clone(DEFAULT),...payload.data};
  }catch(err){
    console.error(err);
  }
  ...
  if(afterState){
    ...
    // Tijdelijke fallback: de demo blijft bruikbaar als de backend even niet bereikbaar is.
    const localGoal=state.goals[pending.goalIndex];
    localGoal.current+=Number(pending.amount);
    state.transactions.unshift({name:pending.name,amount:Number(pending.amount),message:pending.message,goal:localGoal.title,cash,photo:pending.photo||""});
  }

  const currentGoal=state.goals.find(x=>x.id===g.id)||state.goals[state.activeGoal];
  lastDonationAnimation={
    goalIndex:state.goals.findIndex(x=>x.id===currentGoal.id),
    amount:Number(pending.amount),
    before:beforeCurrent,
    after:Number(currentGoal.current),
    goal:Number(currentGoal.goal),
    goalId:currentGoal.id,
    title:currentGoal.title
  };
  save();render();
  document.getElementById("success-title").textContent=`${euro(pending.amount)} toegevoegd 🎉`;
  document.getElementById("success-copy").textContent=cash?`De contante bijdrage staat nu ook digitaal bij ${currentGoal.title}.`:`De Droompot van ${state.childName} is weer een stukje dichter bij ${currentGoal.title}.`;
  document.getElementById("success-balance").textContent=`${euro(currentGoal.current)} van ${euro(currentGoal.goal)}`;
  show("success");startConfetti();
}
document.getElementById("fake-pay-btn").addEventListener("click",e=>{const b=e.currentTarget;b.disabled=true;b.textContent="Betaling verwerken…";setTimeout(async()=>{await complete(false);b.disabled=false},550)});
document.getElementById("confirm-cash-btn").addEventListener("click",async()=>{await complete(true)});
...
document.getElementById("start-dream-btn").addEventListener("click",openLockedDream);
...
document.getElementById("settings-login-btn").addEventListener("click",async()=>{
  ...
  const button=document.getElementById("settings-login-btn");
  button.disabled=true;
  button.textContent="Inloggen…";
  try{
    const response=await fetch("/.netlify/functions/dream-data",{
      method:"POST",
      headers:{"Content-Type":"application/json","X-Dream-User":username,"X-Dream-Password":password},
...
document.querySelectorAll(".nav-item").forEach(b=>b.addEventListener("click",()=>{
  ...
  if(p==="Spaarrekening"){
    ...
    const fakeLink=`https://droompot.nl/r/${state.childName.toLowerCase().replace(/[^a-z0-9]/g,"") || "${state.childName}"}5`;
    extra.innerHTML=`
...
function renderWishlist(){
  ...
  list.innerHTML="";
  const wishes=(state.wishes||[]).filter(w=>wishFilter==="all"||(wishFilter==="claimed"?w.claimed:!w.claimed)).slice().sort((a,b)=>(Number(b.price)||0)-(Number(a.price)||0));
  if(!wishes.length){
...
function renderSettingsWishList(){
  ...
  if(!wrap)return;
  const wishes=(settingsDraft?.wishes || state.wishes || []).slice().sort((a,b)=>(Number(b.price)||0)-(Number(a.price)||0));
  wrap.innerHTML="";
  wishes.forEach(w=>{
  ...
  wrap.querySelectorAll("[data-remove-setting-wish]").forEach(btn=>{
    btn.addEventListener("click",()=>{
      ...
      state.wishes=clone(settingsDraft.wishes);
      save();
      saveCentralState();
      renderSettingsWishList();
    });
...
async function fetchProductMetadata(){
  const urlInput=document.getElementById("setting-wish-url");
  ...
  if(!url){
    status.textContent="Plak eerst een productlink.";
  ...
  try{ new URL(url); }catch{
    status.textContent="Deze productlink lijkt niet geldig.";
    toast("Deze productlink lijkt niet geldig");
    return;
  }
  urlInput.value=url;
  ...
  const originalText=button.textContent;
  button.disabled=true;
  button.textContent="Even ophalen…";
  status.textContent="Naam, prijs en productfoto worden opgehaald…";

  const clean=s=>String(s??"").replace(/\s+/g," ").trim();
  const normalizePrice=value=>{
    if(value===null||value===undefined||value==="")return "";
    let s=String(value).replace(/\u00a0/g," ").trim();
    const match=s.match(/(?:€|EUR)?\s*([0-9]{1,7}(?:[.,][0-9]{1,2})?)/i);
    if(!match)return "";
    let v=match[1];
  ...
  try{
    if(location.protocol==="file:"){
      throw new Error("Open Droompot via START_PREVIEW.bat zodat productgegevens opgehaald kunnen worden.");
    }

    const data=await getLocalMetadata();
    ...
    if(image){
      media.innerHTML=`<img src="${esc(image)}" alt="" referrerpolicy="no-referrer">`;
    }else{
      media.innerHTML=`<img src="droompot-pig.png" alt="Droompot">`;
    }

    const missing=[];
    if(!title)missing.push("naam");
    if(!price)missing.push("prijs");
    if(!image)missing.push("foto");

    if(!missing.length){
      status.textContent="Naam, prijs en productfoto zijn ingevuld.";
      toast("Productgegevens opgehaald",true);
    }else{
      status.textContent=`Niet alles gevonden. Ontbreekt nog: ${missing.join(", ")}.`;
      toast("Productgegevens deels opgehaald");
    }
  }catch(err){
    console.error(err);
    status.textContent=err?.message||"Ophalen lukte niet.";
    toast("Productgegevens ophalen mislukt");
  }finally{
    button.disabled=false;
...
document.getElementById("setting-wish-url")?.addEventListener("keydown",e=>{
  if(e.key==="Enter"){e.preventDefault();fetchProductMetadata();}
});
...
function renderSettingsContributions(){
  ...
  list.forEach((t,i)=>{
    ...
    item.innerHTML=`${photo}
      <div><strong>${esc(t.name)}${isDemoContribution(t)?" (demo)":""} · ${euro(t.amount)}</strong><small>${esc(t.message||"Geen bericht")} · ${esc(t.goal||"Droompot")}</small></div>
...
function startProfileCoinRain(){
  ...
  stopProfileCoinRain();
  const photo=state.photo||"${state.childName}.jpg";
...
function syncDynamicChildNames(){
  document.querySelectorAll(".dynamic-child-name").forEach(el=>{
    el.textContent=(state && state.childName) ? state.childName : "Noï";
  });
}

document.addEventListener("DOMContentLoaded",()=>setTimeout(syncDynamicChildNames,0));

async function shareDroompotLink({wishlist=false}={}){
  const child=state.childName || "Noï";
  const data=wishlist
    ? {
        title:`Verlanglijstje van ${child}`,
        text:`🎁✨ Bekijk het verlanglijstje van ${child} op Droompot! 🐷💛`,
        url:"https://www.droompot.nl"
      }
    : {
        title:`Droompot van ${child}`,
        text:`🐷✨ Bekijk de Droompot van ${child}: spaardoelen, verlanglijstje en sparen voor later 💛`,
        url:"https://www.droompot.nl"
      };
...
document.getElementById("bottom-share-btn")?.addEventListener("click",()=>{
  shareDroompotLink({wishlist:false});
});

document.getElementById("share-wishlist")?.addEventListener("click",()=>{
  shareDroompotLink({wishlist:true});
});
